...
- **Returns without CU-19.** There is no `returns` table: `return_shipments.return_id` is an opaque id that CU-19 must supply together with the buyer account and the store. After creating the logistics reference, CU-19 calls `RegisterReturnShipmentUseCase` (idempotent; leaves the return `PICKUP_PENDING`) and listens to the `ReturnDeliveredToSeller` application event (published in the same transaction) to start the inspection. States are `ReturnStatus` (`PICKUP_PENDING, PICKED_UP, IN_RETURN, LOGISTICS_ISSUE, PICKUP_FAILED, DELIVERED_TO_SELLER`) with the rules in `ReturnTransitions`; the third failed pickup sets `pickup_stopped`, closes the polling, notifies both parties, audits `RETURN_PICKUP_STOPPED` and rejects every later update. The buyer's «new pickup option» (A1) is not implemented: it belongs to CU-19's return-method selection.
...
- CU-19 (returns) does not exist: nothing creates `return_shipments` except its own `RegisterReturnShipmentUseCase`, so return tracking is reachable only for ids that someone registered (the demo seed does it by SQL). Every backend replica runs the polling sweep (safe: deduplication lives in the DB); `stack.yml` does not yet provide the `logistics.webhook.secret` secret, so under Swarm the webhook stays closed until it is created.

### Database

- Flyway owns the schema (`src/main/resources/db/migration/`, currently V1–V21): V1–V5 core (products, carts, addresses, reservations, orders), V6–V7 accounts and JDBC sessions, V8–V9 moderation (reports, cases), V10 password-recovery tokens, V11–V12 `orders.account_id` and wider `status`, V13–V19 CU-23 (stores/snapshot, status history, `audit_events`, shipments, notifications, order issues, cancellations/refunds), V20–V21 CU-24/CU-25 (`shipment_tracking_events` + polling columns on `shipments`; `return_shipments` + `return_tracking_events`). Hibernate runs with `ddl-auto=validate`, so every entity change needs a **new** migration numbered after the highest one on `origin/main`; never edit an applied one and never use `update`/`create`. Another branch may have taken a number: renumber yours when merging (`feature/cu02-recommendations` will need to move its V8).
...
`README.md` (root) and `db/migration/README.md` still say there are no use cases and no V1 migration; that predates the cart/checkout/payment work (schema is at V21). Trust the code and migrations over any older statement. The rest of the README (setup, team rules) is still valid: branch per change (never commit to `main`), merge `origin/main` and run `./mvnw clean verify` before integrating.
